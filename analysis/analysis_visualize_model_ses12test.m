function analysis_visualize_model_ses12test(sj_num, varargin)
...
figbasename = fullfile(preddir, sprintf('%s_predict_rating_ses12test_%s', sj_id, parctype));

%% Time-averaged data

x{1} = reshape(cat(1, rating_dat_tbin{:,1:12,nbin==10}), [10 size(rating_dat_tbin(:,1:12,:),1:2)]);
x{2} = reshape(cat(1, rating_dat_tbin{:,1:12,nbin==5}), [5 size(rating_dat_tbin(:,1:12,:),1:2)]);
x{3} = cellfun(@mean, rating_dat(:,1:12));
x{4} = mean(x{3});
...
for i = 1:numel(x)
    ...
    switch sj_num
        case 1
            switch i
                case 1
                    set(gca, 'XLim', [0.34 0.86], 'YLim', [0.38 0.8], 'XTick', 0.4:0.1:0.8, 'YTick', 0.4:0.1:0.8);
                case {2 3}
        ...
        case 2
            switch i
                case 1
                    set(gca, 'XLim', [0 0.8], 'YLim', [0 0.8], 'XTick', 0:0.2:0.8, 'YTick', 0:0.2:0.8);
                case {2 3 4}
                    set(gca, 'XLim', [0 0.8], 'YLim', [0 0.8], 'XTick', 0:0.4:0.8, 'YTick', 0:0.4:0.8);
            end
    end
