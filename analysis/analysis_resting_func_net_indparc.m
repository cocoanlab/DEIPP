function analysis_resting_func_net_indparc(sj_num, bctgitdir, sctxparc, sctxparclabel, varargin)
...
indparc.cf = ft_read_cifti_mod(func{1}.cf.bold);
indparc.cf.data = [];
indparc.cf.dimord = 'scalar_pos';

ctxparc_cf = ft_read_cifti_mod(fullfile(indnetdir, sprintf('%s_parcels_edgethresh_0.5.dtseries.nii', sj_id)));
[~, ~, ctxparc_cf.data] = unique(ctxparc_cf.data);
ctxparc_cf.data = ctxparc_cf.data - 1; % b/c zeros are changed into ones in the unique step
...
brainstr_nomw = indparc.cf.brainstructure(indparc.cf.brainstructure ~= -1);
indparc.id = zeros(numel(brainstr_nomw), 1);
indparc.id(ismember(brainstr_nomw, 1:2)) = ctxparc_cf.data;
indparc.id(~ismember(brainstr_nomw, 1:2)) = sctxparc_cf.data(~ismember(brainstr_nomw, 1:2));

indparc.thrall = [0.003 0.004 0.005:0.005:0.050];
indparc.thrref = 0.025;

s = load(sctxparclabel); f = fieldnames(s);
sctxparclabeldat = s.(f{1});

indparc.net = importdata(fullfile(indnetdir,sprintf('%s_rawassn_minsize4_regularized_recolored.txt',sj_id)));
indparc.net(indparc.net < 1 | indparc.net > 17) = 0;
indparc.net = [indparc.net; repmat(sctxparclabeldat.dat(:,2)+17, 1, size(indparc.net,2))];
indparc.net = indparc.net + 1; % Start from 1 = None

indparc.net_thrall = importdata(fullfile(indnetdir,sprintf('%s_rawassn_minsize4_regularized_allcolumns_recolored.txt',sj_id)));
indparc.net_thrall(indparc.net_thrall < 1 | indparc.net_thrall > 17) = 0;
indparc.net_thrall = [indparc.net_thrall; repmat(sctxparclabeldat.dat(:,2)+17, 1, size(indparc.net_thrall,2))];
indparc.net_thrall = indparc.net_thrall + 1; % Start from 1 = None

indparc.netlabels = {'NONE', 'DMN', 'LVN', 'FPN', 'MVN', 'DAN', 'PM', 'VAN', 'SAN', 'CON', ...
    'HSMN', 'FSMN', 'AN', 'AMTL', 'PMTL', 'PAM', 'CT', 'LSMN', 'THA', 'HCAMY', 'BG', 'CB', 'BS'};
...
indparc.cf.pos(1:64984,:) = [getfield(gifti(surf_L), 'vertices'); getfield(gifti(surf_R), 'vertices')];

aparcfile = fullfile(fsLRdir, sprintf('%s.aparc.32k_fs_LR.dlabel.nii', anat.basename));
aparccf = ft_read_cifti_mod(aparcfile);
indparc.novis = ~ismember(indparc.net, [3 5]) & ...
    ~ismember(1:numel(indparc.net), unique(indparc.id(find(ismember(aparccf.data, [5 7 11 13 21 40 42 46 48 56]))))).';

save(fullfile(refnetdir, sprintf('%s_resting_func_indparc_meta.mat', sj_id)), 'indparc');

tempcf = indparc.cf;
tempcf.data = indparc.id;
tempcf.mapname = {'ROI number'};
imgname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_roi', sj_id));
ft_write_cifti_mod(imgname, tempcf);

tempcf = indparc.cf;
tempcf.data = indparc.id .* 0 + 1;
tempcf.mapname = {'ROI existence'};
imgname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_binaryroi', sj_id));
ft_write_cifti_mod(imgname, tempcf);

tempcf = indparc.cf;
tempcf.data = indparc.id;
for i = 1:max(indparc.id)
    tempcf.data(indparc.id == i) = indparc.net(i);
end
tempcf.mapname = {'Network labels'};
imgname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_net', sj_id));
ft_write_cifti_mod(imgname, tempcf);

tempcf = indparc.cf;
tempcf.data = indparc.cf.brainstructure(indparc.cf.brainstructure ~= -1);
tempcf.mapname = {'Brainstructure labels'};
imgname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_brainstructure', sj_id));
ft_write_cifti_mod(imgname, tempcf);
system(sprintf('wb_command -cifti-separate %s.dscalar.nii COLUMN -volume-all %s.nii.gz', imgname, imgname));

tempcf = indparc.cf;
tempcf.data = double(ismember(indparc.id, find(~indparc.novis)));
tempcf.mapname = {'ROI in the visual network and occipital areas'};
imgname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_binaryroivis', sj_id));
ft_write_cifti_mod(imgname, tempcf);
...
for ses_i = 1:numel(funcmetafile)    
    ...
    rf_ts{ses_i} = splitapply(@(x) mean(x,2), rf_dat, indparc.id.');
end
save(fullfile(refnetdir, sprintf('%s_resting_func_indparc_timeseries.mat', sj_id)), 'rf_ts');

%% Prepare connectivity

load(fullfile(refnetdir, sprintf('%s_resting_func_indparc_meta.mat', sj_id)), 'indparc');
load(fullfile(refnetdir, sprintf('%s_resting_func_indparc_timeseries.mat', sj_id)), 'rf_ts');
...
set(gcf, 'Color', 'w');
figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_corr_session_similarity.pdf', sj_id));
pagesetup(gcf); saveas(gcf, figname);
close all;

%% Network attributes

Eglob = NaN(numel(indparc.thrall), numel(rf_corr));
ModQ = NaN(numel(indparc.thrall), numel(rf_corr));
for thr_i = 1:numel(indparc.thrall)
    for ses_i = 1:numel(rf_corr)
        r = threshold_proportional(tanh(rf_corr{ses_i}), indparc.thrall(thr_i));
        Eglob(thr_i,ses_i) = efficiency_wei(r);
        ModQ(thr_i,ses_i) = calc_modularity(r, indparc.net_thrall(:,thr_i));
    end
end
save(fullfile(refnetdir, sprintf('%s_resting_func_indparc_netattr.mat', sj_id)), 'Eglob', 'ModQ');

%% Correlation: Network attribute - Pain PC

load(fullfile(refnetdir, sprintf('%s_resting_func_indparc_netattr.mat', sj_id)), 'Eglob', 'ModQ');
...
for netattr = {Eglob ModQ; 'Eglob' 'ModQ'; 'Global efficiency' 'Modularity Q'}
    ...
    set(gcf, 'Color', 'w');
    figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_corr_%s_painPC.pdf', sj_id, netattr{2}));
    pagesetup(gcf); saveas(gcf, figname);
    close all;
end
...
r = tanh(rf_corr_all);
vis_grp_r(r, indparc.net, rcols.power);
clim_val = extlim(r);
fprintf('Clim: [%.4f, %.4f]\n', clim_val(:));
caxis(clim_val);
colormap(rcols.bipolar);
...
figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_rawconn_ses-all.pdf', sj_id));
pagesetup(gcf); saveas(gcf, figname);
close all;

r = tanh(grpmean_r(rf_corr_all, indparc.net));
vis_grp_r(r, [], rcols.power(unique(indparc.net),:));
clim_val = extlim(r);
fprintf('Clim: [%.4f, %.4f]\n', clim_val(:));
caxis(clim_val);
colormap(rcols.bipolar);
...
figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_netconn_ses-all.pdf', sj_id));
pagesetup(gcf); saveas(gcf, figname);
close all;
...
colorbar('TickLength', 0);
resize_axes(gca, [700, 700]);
figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_corr_rawconn_painPC.pdf', sj_id));
pagesetup(gcf); saveas(gcf, figname);
close all;

x = cellfun(@(x) grpmean_r(x, indparc.net), rf_corr, 'un', false);
...
colorbar('TickLength', 0);
resize_axes(gca, [500, 500]);
figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_corr_netconn_painPC.pdf', sj_id));
pagesetup(gcf); saveas(gcf, figname);
close all;
...
xy_ref = repmat({xy_net(indparc.net, 1:2)}, 1, numel(indparc.thrall)+1);

for thr_i = 1:numel(indparc.thrall)
    A = threshold_proportional(tanh(rf_corr_all), indparc.thrall(thr_i));
    ...
    scatter(h_graph_xy{1}, h_graph_xy{2}, 100, rcols.power(indparc.net(wh_largecomp), :), 'MarkerFaceColor', 'flat', ...
    ...
    annotation(gcf, 'textbox', [0 0 1 1], 'String', sprintf('%s\nTop %.1f%%', sj_id, indparc.thrall(thr_i)*100), 'EdgeColor', 'none', 'FontSize', 16);
    
    pagesetup(gcf);
    figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_spring_embed_%s.pdf', sj_id, strrep(sprintf('%.3f', indparc.thrall(thr_i)),'.','_')));
    saveas(gcf, figname);
    figname = fullfile(refnetdir, sprintf('%s_resting_func_indparc_spring_embed_%s.png', sj_id, strrep(sprintf('%.3f', indparc.thrall(thr_i)),'.','_')));
    saveas(gcf, figname);
    close all;
end

for ses_i = 1:numel(rf_corr)
    A = threshold_proportional(tanh(rf_corr{ses_i}), indparc.thrref);
    [A_comps, A_comp_sizes] = get_components(A);
    wh_largecomp = ismember(A_comps, find(A_comp_sizes > floor(size(A,1) * 0.05)));
    G = graph(A(wh_largecomp, wh_largecomp));
    h_graph = plot(G, 'Layout', 'force', ...
        'WeightEffect', 'inverse', 'UseGravity', true, 'Iterations', 1000, ...
        'XStart', xy_ref{find(indparc.thrall==indparc.thrref)+1}(wh_largecomp,1), 'YStart', xy_ref{find(indparc.thrall==indparc.thrref)+1}(wh_largecomp,2));
    h_graph_xy = {h_graph.XData, h_graph.YData};
    h_graph_xylim = {xlim, ylim};
    close all; figure; hold on;
    line(h_graph_xy{1}(G.Edges.EndNodes).', h_graph_xy{2}(G.Edges.EndNodes).', 'Color', [0.6 0.6 0.6 0.7]);
    ...
    set(gca, 'XLim', h_graph_xylim{1}, 'YLim', h_graph_xylim{2}, 'Visible', 'off');
    set(gcf, 'Position', [420         104        1077         875], 'Color', 'w');
    annotation(gcf, 'textbox', [0 0 1 1], 'String', sprintf('%s, ses-%.2d\nTop %.1f%%', sj_id, ses_i, indparc.thrref*100), 'EdgeColor', 'none', 'FontSize', 16);
    ...
    pagesetup(gcf);
    figname = fullfile(refnetdir, sprintf('%s_ses-%.2d_resting_func_indparc_spring_embed_%s.pdf', sj_id, ses_i, strrep(sprintf('%.3f', indparc.thrref),'.','_')));
    saveas(gcf, figname);
    figname = fullfile(refnetdir, sprintf('%s_ses-%.2d_resting_func_indparc_spring_embed_%s.png', sj_id, ses_i, strrep(sprintf('%.3f', indparc.thrref),'.','_')));
    saveas(gcf, figname);
    close all;
end
