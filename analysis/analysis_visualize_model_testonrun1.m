function analysis_visualize_model_testonrun1(sj_num, varargin)
...
figbasename = fullfile(preddir, sprintf('%s_predict_rating_testonrun1_%s', sj_id, parctype));

%% Time-averaged data

x{1} = reshape(cat(1, rating_dat_tbin{1,:,nbin==10}), [10 size(rating_dat_tbin,2)]);
x{2} = reshape(cat(1, rating_dat_tbin{1,:,nbin==5}), [5 size(rating_dat_tbin,2)]);
x{3} = cellfun(@mean, rating_dat(1,:));

y{1} = reshape(cat(1, Yfit_tbin{1,:,nbin==10}), [10 size(Yfit_tbin,2)]);
y{2} = reshape(cat(1, Yfit_tbin{1,:,nbin==5}), [5 size(Yfit_tbin,2)]);
y{3} = cellfun(@mean, Yfit(1,:));
