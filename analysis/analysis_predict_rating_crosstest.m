function analysis_predict_rating_crosstest(tesj_num, trsj_num, varargin)
...
for task_i = 1:numel(tasklabels)
    funcmetafile = fullfile(metadir, sprintf('%s_%s_%s_%s_metadata.mat', tesj_id, 'ses-*', 'func', tasklabels{task_i}));
...
mdlname = fullfile(trpreddir, sprintf('%s_predict_rating_modeling_%s.mat', trsj_id, parctype));
...
for task_i = 1:size(func,1)
    ...
    for ses_i = 1:size(func,2)
        ...
        crte_Yfit{task_i, ses_i} = func_conn * w(2:end) + w(1);
        
        for bin_i = 1:numel(nbin)

            crte_Yfit_rbin{task_i,ses_i,bin_i} = splitapply(@(x) mean(x,1), crte_Yfit{task_i, ses_i}, rbin_idx{task_i,ses_i,bin_i});
            crte_Yfit_tbin{task_i,ses_i,bin_i} = splitapply(@(x) mean(x,1), crte_Yfit{task_i, ses_i}, tbin_idx{task_i,ses_i,bin_i});
