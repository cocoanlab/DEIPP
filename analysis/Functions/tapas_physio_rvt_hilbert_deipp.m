function [rvt, rr, verbose] = tapas_physio_rvt_hilbert_deipp(fr, t, sample_points, verbose)
...
%
% Modified by Jae-Joong Lee, 2023. 07. 01
% : Respiratory rate as optional output
...
%% Calculate respiratory rate (NEW) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

n_win = 10 * f_sample / 2;
n_win_half = floor(n_win / 2);

idx_win_post = [1+n_win_half:numel(fr_phase) repmat(numel(fr_phase),1,n_win_half)].';
idx_win_pre = [repmat(1,1,n_win_half) 1:numel(fr_phase)-n_win_half].';
fr_phase_diff = fr_phase(idx_win_post) - fr_phase(idx_win_pre);
fr_rr = fr_phase_diff ./ (2 * pi) ./ (idx_win_post-idx_win_pre) .* f_sample;

f_sample_out = 1 / mean(diff(sample_points));
[re_rr, t_re_rr] = resample(fr_rr, t, f_sample_out);
% And now interpolate onto new timepoints
rr = interp1(t_re_rr, re_rr, sample_points, 'linear');
% Nearest-neighbour interpolation for outside recorded timepoints
% Be more careful here as don't want rr to go negative
if sum(isnan(rr)) > 0
    nan_inds = isnan(rr);
    rr(nan_inds) = interp1( ...
        t_re_rr, re_rr, sample_points(nan_inds), ...
        'nearest', 'extrap');
end
rr = rr .* 60;
