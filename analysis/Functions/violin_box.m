function [handles, dot_locs] = violin_box(x, varargin)

% Draw a violin and box plot
%
% Usage:
% -------------------------------------------------------------------------
% violin_box(x, varargin)
%
% Inputs:
% -------------------------------------------------------------------------
...
% end
%
% Modified from 'boxplot_wani_2016.m' from CocoanCORE
% 2023. 06. 10
...
if dodots
    for i = 1:numel(xvalues)
        if ~use_onedotcolor
            h = scatter(xvalues{i}, x_cell{i}, dot_size, colud3(i,:), 'filled', 'MarkerFaceAlpha', dot_alpha);
        else
            h = scatter(xvalues{i}, x_cell{i}, dot_size, data_dotcolor{i}, 'filled', 'MarkerFaceAlpha', dot_alpha);
        end
    end
    handles.dots = h;
end
