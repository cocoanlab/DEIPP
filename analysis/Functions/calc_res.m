function s = calc_res(Y, Yfit, idx_hout)
...
for j = 1:size(Y, 2)
    ...
    if ismember(j, idx_train)
        ma = mean(cat(1, Y{:, setdiff(idx_train, j)}));
    else
        ma = mean(cat(1, Y{:, idx_train}));
    end
...
[a, b] = deal(cat(1, Y{:,idx_train}), cat(1, Yfit{:,idx_train}));
if all(isnan(a) | isnan(b))
    [a, b] = deal(NaN);
else
    [a, b] = deal(a(~isnan(a) & ~isnan(b)), b(~isnan(a) & ~isnan(b)));
...
if ~isempty(idx_hout)
    [a, b] = deal(cat(1, Y{:,idx_hout}), cat(1, Yfit{:,idx_hout}));
    if all(isnan(a) | isnan(b))
        [a, b] = deal(NaN);
    else
        [a, b] = deal(a(~isnan(a) & ~isnan(b)), b(~isnan(a) & ~isnan(b)));
