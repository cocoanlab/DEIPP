function analysis_visualize_model_test(sj_num, varargin)
...
figbasename = fullfile(preddir, sprintf('%s_predict_rating_test_%s', sj_id, parctype));
    ...
    switch sj_num
        ...
        case 2
            switch i
                case 1
                    switch parctype
                        case {'indparc', 'indparcwvis', 'schaefer'}
                            set(gca, 'XLim', [0 0.9], 'YLim', [0 0.9], 'XTick', 0:0.2:0.8, 'YTick', 0:0.2:0.8);
                        case 'indparconlyvis'
                            set(gca, 'XLim', [-0.1 0.9], 'YLim', [-0.1 0.9], 'XTick', 0:0.2:0.8, 'YTick', 0:0.2:0.8);
                    end
                case {2 3 4}
...
rrun_r = cellfun(@(a,b) corr(a(:), b(:)), squeeze(num2cell(x{1},1)), squeeze(num2cell(y{1},1)));
rrun_mr = mean(rrun_r(:));
rrun_mboot = mean(rrun_r(boot_idx{3}));
...
for ses_i = 1:size(x{1},3)
    for task_i = 1:size(x{1},2)
        sccol = interp1(linspace(-1,1,size(cols.bipolar,1)), cols.bipolar, rrun_r(task_i,ses_i));
...
rses_r = cellfun(@(a,b) corr(a(:), b(:)), squeeze(num2cell(x{1},1:2)), squeeze(num2cell(y{1},1:2)));
rses_mr = mean(rses_r);
rses_mboot = mean(rses_r(boot_idx{4}));
...
hold on;
for ses_i = 1:size(x{1},3)
    sccol = [interp1(linspace(-1,1,size(cols.bipolar,1)), cols.bipolar, rses_r(ses_i))];
