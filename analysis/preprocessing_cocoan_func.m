...
func.cf.fwhm = 6;
func.cf.bold = fullfile(cfdir, anat.basename, 'MNINonLinear', 'Results', func.basename, ...
    sprintf('%s_Atlas_s%d.dtseries.nii', func.basename, func.cf.fwhm));
if ~exist(func.cf.bold, 'file') || do_override
    system(sprintf(['source ~/.ciftifyvenv/bin/activate;', ...
        ...
        cfgitdir, cfgitdir, cfgitdir, cfdir, func.cf.fwhm, func.den.boldmean, func.den.bold, anat.basename, func.basename));
    system(sprintf(['source ~/.ciftifyvenv/bin/activate;', ...
        'export PATH=$PATH:%s/ciftify/bin;', ...
        'export PYTHONPATH=$PYTHONPATH:%s;', ...
        'export CIFTIFY_TEMPLATES=%s/ciftify/data;', ...
        'cifti_vis_fmri snaps --ciftify-work-dir %s --SmoothingFWHM %d --meanfunc %s --verbose %s %s'], ...
        cfgitdir, cfgitdir, cfgitdir, cfdir, func.cf.fwhm, func.den.boldmean, func.basename, anat.basename));
    system(sprintf('rm %s.nii.gz', fullfile(cfdir, anat.basename, 'MNINonLinear', 'Results', func.basename, func.basename)));
end
